<template>
  <div class="notif-page">
    <nav class="notif-bc" aria-label="面包屑">
      ...
      <span class="notif-bc-current">消息通知</span>
    </nav>

    <div class="notif-layout">
      <section class="notif-panel" aria-label="消息通知">
        <header class="notif-topbar">
          <div class="notif-topbar-intro">
            <h2 class="notif-topbar-title">消息中心</h2>
            <p class="notif-topbar-hint">站内提醒与互动通知</p>
          </div>
          <nav class="notif-nav" role="tablist" aria-label="消息分类">
            <button
              type="button"
              role="tab"
        ...
        <header class="notif-header">
          ...
          <div class="notif-header-actions">
            <button v-if="list.length" type="button" class="notif-btn notif-btn--ghost" @click="markAll">
...
const AVATAR_COLORS = ['#3b82f6', '#0ea5e9', '#2563eb', '#6366f1', '#8b5cf6']
...
async function onItemClick(item) {
  ...
  router.push({ name: 'AdminNotificationDetail', params: { id: String(nid) } })
}
...
watch(
  () => route.name,
  (name, oldName) => {
    if (name === 'AdminNotifications' && oldName === 'AdminNotificationDetail') {
      load()
    }
  }
)
